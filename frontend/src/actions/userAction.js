...
export const register = (userData) => async (dispatch) => {
    try {
        ...
        //useful if the calling function needs this data
...
export const logout = () => async (dispatch) => {
    try {
        ...
        dispatch({ type: CLEAR_CART }); //Clear cart when user logged out
    } catch (error) {
        dispatch({
            type: LOGOUT_FAIL,
            payload: error.response.data.message,
...
export const updatePassword = (passwords) => async (dispatch) => {
    try {
        ...
        // const { data } = await axios.post("/api/v1/users/password/update", passwords, config);
        dispatch({
